...
void BiquadFilterNode::getFrequencyResponse(
  ...
  updateCoefficientsForFrame(
      frequencyParam_->getValue(),
      detuneParam_->getValue(),
      QParam_->getValue(),
      gainParam_->getValue());
...
BiquadFilterNode::FilterFrameParams BiquadFilterNode::getFrameParams(
    int framesToProcess) const {
  ...
  return {
      ...
      detuneParam_->processARateParam(framesToProcess, currentTime)
          ->getChannel(0)
          ->getData(),
      QParam_->processARateParam(framesToProcess, currentTime)
          ->getChannel(0)
          ->getData(),
      gainParam_->processARateParam(framesToProcess, currentTime)
          ->getChannel(0)
          ->getData()};
}

void BiquadFilterNode::processChannel(
    float *channelData,
    int framesToProcess,
    const BiquadFilterNode::FilterFrameParams &params,
    bool last) {
  float x1 = x1_;
  float x2 = x2_;
  float y1 = y1_;
  ...
  float prevFreq = params.frequency[0] - 2e-6f;
  float prevDetune = params.detune[0] - 2e-6f;
  float prevQ = params.q[0] - 2e-6f;
  float prevGain = params.gain[0] - 2e-6f;

  for (int i = 0; i < framesToProcess; i++) {
    float freq = params.frequency[i];
    float detune = params.detune[i];
    float q = params.q[i];
    float gain = params.gain[i];

    if (needsUpdate(
            freq, detune, q, gain, prevFreq, prevDetune, prevQ, prevGain)) {
      updateCoefficientsForFrame(freq, detune, q, gain);
      prevFreq = freq;
      prevDetune = detune;
      prevQ = q;
      prevGain = gain;
...
void BiquadFilterNode::processNode(
    ...
    int framesToProcess) {
  auto params = getFrameParams(framesToProcess);
  for (int c = 0; c < processingBus->getNumberOfChannels(); c++) {
    ...
    processChannel(
        ...
        framesToProcess,
        params,
        c == processingBus->getNumberOfChannels() - 1);
  }
}
